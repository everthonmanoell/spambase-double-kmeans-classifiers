...
O projeto exige a implementação de um classificador bayesiano utilizando a estimativa de densidade por k-vizinhos e a estimativa de máxima verossimilhança para a probabilidade a priori das classes $P(\omega_i)$[cite: 11]. Matematicamente, a regra de classificação padrão do algoritmo k-NN é idêntica à formulação bayesiana. 
...
Onde $k_i$ é o número de vizinhos da classe $\omega_i$ dentro da vizinhança de volume $V$, e $N_i$ é o total de exemplos da classe. Utilizando a estimativa de máxima verossimilhança para a priori ($P(\omega_i) = \frac{N_i}{N}$)[cite: 11], e aplicando o Teorema de Bayes para a probabilidade a posteriori:
...
* **Métrica de Distância (`metric`):** Em estrito atendimento ao documento do projeto, a grade de busca fixará a vizinhança testando as distâncias Euclidiana, City-Block (conhecida como Manhattan na biblioteca) e Chebishev[cite: 11].
